...
from app.service import teacherService, studentService
from app.utils import uploadUtil

upload_file_api = APIRouter()


@upload_file_api.post("/teacher/avatar", summary="教师用户头像上传")
async def teacher_avatar_upload(request: Request, avatar_file: UploadFile = File(...)):
...
@upload_file_api.post("/student/avatar", summary="学生用户头像上传")
async def student_avatar_upload(request: Request, avatar_file: UploadFile = File(...)):
    path = await rename_and_upload(avatar_file, "student/avatar")

    student_id = request.state.user_id
    # 更新数据库
    await studentService.update_student_avatar(avatar=path, student_id=student_id)
